...
const CIRCLE_ICONS = {
    purple: `<path d="M12 3v18M3 12h18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>`,
    green:  `<circle cx="12" cy="12" r="3" stroke="currentColor" stroke-width="1.5"/>
             <path d="M12 2v4m0 12v4m10-10h-4M6 12H2" stroke="currentColor" stroke-width="1.5"/>`,
    yellow: `<path d="M9 18V5l12-2v13" stroke="currentColor" stroke-width="1.5"/>
             <circle cx="6" cy="18" r="3" stroke="currentColor" stroke-width="1.5"/>`,
    pink:   `<path d="M4 19.5A2.5 2.5 0 016.5 17H20" stroke="currentColor" stroke-width="1.5"/>
...
window.renderCircles = function(data) {
    ...
    const cardsHTML = data.map((c, i) => {
        ...
        const btnHtml = isClosed
            ...
            : `<button class="circle-enroll-btn" onclick="window.openEnroll(${i})">${window.t('btn.enroll')} <svg class="circle-btn-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg></button>`;

        const iconMarkup = CIRCLE_ICONS[c.color] || CIRCLE_ICONS.purple;
        const iconMarkupWhite = iconMarkup.replace(/stroke="currentColor"/g, 'stroke="#fff"');
